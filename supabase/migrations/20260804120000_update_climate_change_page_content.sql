-- Rebuilds the "Climate Change & Trees" CMS page content to match the
-- design template (screenshots) supplied for this page: a hero with the
-- "Read on to discover..." headline, a Contact-us CTA that links through to
-- the existing /contact page, a 3-column "climate crisis" section, and a
-- full-bleed "plant more trees" section. Uses upsert-by-slug so this applies
-- whether the page was already seeded (via seed-default-pages) or not yet.
