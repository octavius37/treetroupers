-- Backup of the six public CMS page rows, taken 2026-08-25 from the hosted
-- project (ref lsjoxlmpciuzgxtqtjmf) immediately before they were retired in
-- favour of hand-written route files under app/pages/.
--
-- Every `content` value below was verified byte-identical to the live row by
-- md5 comparison (hashes recorded per row). This file is the restore path if
-- the code pages ever need to be rolled back; it is not applied by any
-- migration and never runs automatically.
--
-- Restore with: psql <conn> -f 2026-08-25-pages-backup.sql

-- home — md5 a28df7a3c807a080bd414874929a7afa, 3600 chars
...
-- who-we-are — md5 3b4fcd9554d9d37a77ae2dcfcf9abc60, 3781 chars
insert into public.pages (title, slug, content, status, show_in_nav, nav_order)
values (
  'Who We Are',
  'who-we-are',
...
-- mission — md5 93aa848d243b4abf2f7207983d82f00e, 3755 chars
insert into public.pages (title, slug, content, status, show_in_nav, nav_order)
values (
  'Our Mission',
  'mission',
  $page$<body><section class="py-20 px-4 max-w-4xl mx-auto"><div id="iwg7i"><b>The Climate Emergency</b></div><div id="iez8h" class="gjs-row"><div class="gjs-cell"></div></div><div id="is0ci" class="gjs-row"><div class="gjs-cell"><div id="ib93e"><span id="irdxf">It is difficult to
overstate the seriousness of the climate crisis and the urgency of taking
action to mitigate its effects. The burning of fossil fuels is driving
unprecedented and accelerating changes in the Earth's climate system, with the
critical 1.5°C global temperature increase threshold breached in 2024. Without
immediate and deep emissions cuts, the long-term average is likely to surpass
1.5°C within this decade, posing direct threats to human well-being and
planetary health: more frequent extreme weather events, rising sea levels,
serious disruption to ecosystems and biodiversity, and widespread risks to food
and water security.</span></div></div></div></section><section class="py-16 px-4"><b id="ipkyz">Harnessing Climate Helplessness</b><div class="max-w-5xl mx-auto"></div><div id="i7ooi">Many people do not just worry about climate change — they feel a sense of helplessness about their ability to change the status quo. A substantial proportion of people believe governments are failing to respond adequately, but also that they are personally doing too little to address climate change.<br id="ih222-2" draggable="true"/><br id="ies8a-2" draggable="true"/>Enter Tree Troupe.<br id="ix38q-2" draggable="true"/><br id="i7g2k-2" draggable="true"/>Tree Troupe allows individuals to maximize and collate scaleable personal actions that can meaningfully influence the trajectory of the climate crisis. Tree Troupers harness collective helplessness to drive climate ambition. Tree Troupe is founded on three key tenets:<br id="ixenw-2" draggable="true"/><ol id="ij9e9-2" draggable="true"><li id="izn4b-2" draggable="true">Planting trees is one of the easiest, most effective ways to combat climate change;</li><li id="irbrg-2" draggable="true">Tree planting on a mass scale could be a game-changer for our planet’s health</li><li id="ii0us-2" draggable="true" class="gjs-hovered">Tree-planting is something anyone can do - individually, collectively or globally.<span id="icfih"></span></li></ol></div></section><div id="i3rae" class="gjs-row"><div class="gjs-cell"></div></div><div data-block="communities-carousel" class="my-8 py-12 px-6 rounded-2xl border-2 border-dashed border-green-400 bg-green-50 text-center"><span class="block text-xs uppercase tracking-wider text-green-700 font-semibold mb-2">Smart Block · Communities Carousel</span><span class="block text-sm text-gray-600">Shows active communities from Supabase</span><span class="block text-xs text-gray-400 mt-2">Renders live data on the public page.</span></div><div data-block="stats-counter" class="my-8 py-12 px-6 rounded-2xl border-2 border-dashed border-green-400 bg-green-50 text-center"><span class="block text-xs uppercase tracking-wider text-green-700 font-semibold mb-2">Smart Block · Stats Counter</span><span class="block text-sm text-gray-600">Shows live tree, community &amp; member counts</span><span class="block text-xs text-gray-400 mt-2">Renders live data on the public page.</span></div><div id="itqxg" class="gjs-row"><div id="i546a" class="gjs-cell"><div id="iqqaf"></div></div></div><section class="py-20 px-4 bg-green-600"><div class="max-w-3xl mx-auto text-center"><h2 class="text-3xl font-bold text-white mb-6">Join Tree Troupe Now </h2><p class="text-green-100 text-lg mb-8">Join Forces with Likeminded People!</p><a href="/register" class="inline-block bg-white text-green-700 px-8 py-3 rounded-full font-medium hover:bg-green-50 transition-colors">Get Started</a></div></section></body>$page$,
  'published',
  true,
  0
)
...
-- climate-change — md5 e591292386336f0c82eacfb389e209f2, 5060 chars
insert into public.pages (title, slug, content, status, show_in_nav, nav_order)
values (
  'Climate change & trees',
  'climate-change',
  $page$<section class="relative h-[400px] overflow-hidden bg-gradient-to-b from-sky-300 via-sky-100 to-slate-200">
...
-- what-can-i-do — md5 d4e53a85a426cad5b58b795a4cb61dbb, 6369 chars
insert into public.pages (title, slug, content, status, show_in_nav, nav_order)
values (
  'What Can I Do?',
  'what-can-i-do',
...
-- global-tree-planting-organizations — md5 4c89d6faa2bed89b13840ca33e2ccc18, 5566 chars
insert into public.pages (title, slug, content, status, show_in_nav, nav_order)
values (
